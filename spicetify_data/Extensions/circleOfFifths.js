...
// NAME: CircleOfFifthsShuffle
// AUTHOR: aviously25
// DESCRIPTION: shuffle your playlist using the circle of fifths

//<reference path="../globals.d.ts" />
(function CircleOfFifths() {
    if (!Spicetify.CosmosAPI || !Spicetify.BridgeAPI) {
        setTimeout(CircleOfFifths, 1000);
        return;
    }
    ...
    function createContextButton() {
        ...
        b.innerText = "Shuffle CircleOfFifths";
        b.setAttribute(
            "data-tooltip",
            ...
            fetchListFromUri(contextURI)
                .then((list) => {
                    if (list && list.length > 0) {
                        shuffle(list, setQueue);
                    }
                })
                .catch((err) => {
                    Spicetify.showNotification(`${err}`)
                });
        };
        return b;
    }
    ...
    const cntxMenu = new Spicetify.ContextMenu.Item(
        "Play with CircleOfFifths Shuffle",
        (uris) => {
            if (uris.length === 1) {
     ...
     */
    async function shuffle(array, callback) {
        let counter = 0;
        let success = 0;
        let newArray = [];
        let sorted = false;
        ...
        while (counter < array.length && counter < 11) {
            let songKey = -1;

            //get audio analysis data for each track
            try{
                const audioData = await Spicetify.getAudioData(array[counter].uri);
                songKey = audioData.track.key
                console.log(audioData.track.key)
            } catch(err){
                console.log(err);
            }
            ...
            newArray[counter].key = songKey;

            // And swap the last element with it
            //let temp = array[array.length-counter];
            //array[array.length-counter] = array[index];
            //array[index] = temp;
        }


        callback(sortCircleFifthArray(newArray));
    }

    function sortCircleFifthArray(array){
        console.log(array);
